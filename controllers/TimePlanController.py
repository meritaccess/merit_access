...
class TimePlanController:
    ...
    class DayPlan:
        """
        Data structure to represent a daily schedule with two time periods.
        """
        ...
        def __init__(
            ...
            self.first_start: datetime.time = datetime.strptime(
                first_start, "%H:%M:%S"
            ).time()
            self.first_end: datetime.time = datetime.strptime(
                first_end, "%H:%M:%S"
            ).time()
            self.second_start: datetime.time = datetime.strptime(
                second_start, "%H:%M:%S"
            ).time()
            self.second_end: datetime.time = datetime.strptime(
                second_end, "%H:%M:%S"
            ).time()
    ...
    class TimePlan:
        """
        Data structure to represent a weekly schedule with daily plans and associated actions.
        """
        ...
        def __init__(self) -> None:
            ...
            self.days: Dict[str, TimePlanController.DayPlan] = {
                "mon": None,
                "tue": None,
                "wed": None,
                "thu": None,
    ...
    def parse_tplan(self, tplan: Tuple[str, ...]) -> None:
        ...
        """
        try:
            tplan_obj = TimePlanController.TimePlan()
            ...
            for day in tplan_obj.days.keys():
                for j in range(4):
                    if not self._validate_time_str(tplan[i + j]):
                        ...
                        return
                tplan_obj.days[day] = TimePlanController.DayPlan(
