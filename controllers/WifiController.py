...
class WifiController:
    ...
    def _wifi_on(self) -> None:
        ...
        try:
            ...
            log(10, f"WiFi {self._interface} turned on.")
        except Exception as e:
            err = f"Error turning on WiFi for {self._interface}: {e}"
            log(40, err)
    ...
    def _wifi_off(self) -> None:
        ...
        try:
            ...
            log(10, f"WiFi {self._interface} turned off.")
        except Exception as e:
            err = f"Error turning off WiFi for {self._interface}: {e}"
            log(40, err)

    def wifi_connect(self) -> None:
        """
        Connects to the WiFi network with the provided SSID and password.
        """
        try:
            ...
            err = f"Error connecting to WiFi {self._wifi_ssid}: {e}"
            log(40, err)
    ...
    def _get_available_wifi(self) -> List[str]:
        ...
        try:
            ...
            err = f"Error getting available WiFi networks: {e}"
            log(40, err)
            return []
    ...
    def wifi_disconnect(self) -> None:
        ...
        try:
            ...
            err = f"Error disconnecting from WiFi {self._wifi_ssid}: {e}"
            log(40, err)
    ...
    def ap_on(self) -> None:
        ...
        """
        try:
            self._purge_hotspot_interfaces()
            if not self.check_wifi_status():
                self._wifi_on()
            subprocess.run(
                [
            ...
            err = f"Error turning on Access Point {self._ap_ssid}: {e}"
            log(40, err)
    ...
    def ap_off(self) -> None:
        ...
        try:
            ...
            err = f"Error turning off Access Point {self._ap_ssid}: {e}"
            log(40, err)

    def _purge_hotspot_interfaces(self):
        """
        Purges any existing hotspot interfaces.
        """
        try:
            interfaces = []
            result = subprocess.run(
            ...
            if result.stdout:
                interfaces = list(result.stdout.strip().split("\n"))
            for interface in interfaces:
                subprocess.run(["sudo", "nmcli", "con", "del", interface], check=True)

        except Exception as e:
            err = f"Error purging hotspot interfaces: {e}"
            log(40, err)
    ...
    def check_connection(self) -> bool:
        ...
        try:
            ...
            err = f"Error checking connection for {self._interface}: {e}"
            log(40, err)
            return False
    ...
    def check_wifi_status(self) -> bool:
        ...
        try:
            ...
            err = f"Error checking WiFi status for {self._interface}: {e}"
            log(40, err)
            return False
    ...
    def check_wifi_connection(self):
        ...
        try:
            ...
            log(10, f"Failed to check Wi-Fi connection: {e}")
            return False
